% function [time_exp,Vout,Vin,Vratio_exp,Del_Vout,Del_Phase] = GetExpData(fileName,filename_short,pathname,timeCorrection)
function [time_exp,Vout,Vin,Vratio_exp,Del_Vout,Del_Phase] = GetExpData(fileName,filename_short,pathname,timeCorrection,plotFlag)
% plotFlag = 1 → 그래프 보여줌, plotFlag = 0 → 그래프 안 보여줌
   ...
   % ExpData = fscanf(file_exp,'%10f %10f %10f %10f %10f',[5,1000]);
    ...
    % ===== Simple time-zero correction (-20 ~ -5 ps baseline) =====
    % 1. baseline (-20 ~ -5 ps 평균)
    baseline = mean(Vin(time_exp >= -20 & time_exp <= -5));
    % 2. peak
    [Vin_max, idx_peak] = max(Vin);
    % 3. half level
    half_val = (Vin_max + baseline)/2;
    % 4. leading edge에서 half crossing 찾기 (interpolation)
    t_half = interp1(Vin(1:idx_peak), time_exp(1:idx_peak), half_val);
    % 5. time shift
    time_exp = time_exp - t_half;
